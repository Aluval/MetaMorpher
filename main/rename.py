...
def generate_subtitle(watermark_text, subtitle_path):
    subtitle_content = f"""
[Script Info]
; Script generated by Pyrogram Bot
Title: Watermark Subtitle
ScriptType: v4.00+
WrapStyle: 0
...
async def add_watermark_command(bot, msg):
    if not msg.reply_to_message:
        return await msg.reply_text("Please reply to a media file with the watermark command and specify the watermark text.\nFormat: `/addwatermark Your Watermark Text`")

    if len(msg.command) < 2:
        return await msg.reply_text("Please provide the watermark text.\nFormat: `/addwatermark Your Watermark Text`")
    ...
    c_time = time.time()
    try:
        media_path = await reply.download(progress=progress_message, progress_args=("🚀 Downloading media... ⚡️", sts, c_time))
    except Exception as e:
        await sts.edit(f"Error downloading media: {e}")
        return

    subtitle_path = os.path.join(DOWNLOAD_LOCATION, "watermark.ass")
    generate_subtitle(watermark_text, subtitle_path)

    output_file = os.path.join(DOWNLOAD_LOCATION, "output_" + os.path.basename(media_path))
